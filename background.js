 ...
 */
const RMPProvider = {
  SCHOOL_ID: "U2Nob29sLTEyNTI=", // University of North Texas
  ...
  async search(lastName, firstName) {
    ...
    try {
      ...
      // Filter to only UNT professors
      const schoolProfessors = edges.filter(edge => {
        const schoolName = edge.node.school?.name || '';
        const isMatch = schoolName.toLowerCase().includes('university of north texas');
        if (!PRODUCTION_MODE) console.log(`[RMP API] Checking: ${edge.node.firstName} ${edge.node.lastName} at ${schoolName} - Match: ${isMatch}`);
        return isMatch;
      });

      if (!PRODUCTION_MODE) console.log(`[RMP API] Found ${schoolProfessors.length} UNT professors out of ${edges.length} total results`);

      if (schoolProfessors.length === 0) {
        return { found: false, message: 'No UNT professors found in results' };
      }

      // Try exact match first (in UNT professors only)
      ...
      // Use best match if last name matches (in UNT professors only)
