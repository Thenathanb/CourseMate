 ...
 */
const RMPProvider = {
  SCHOOL_ID: "U2Nob29sLTE1MTY=", // University of Texas at San Antonio
  ...
  async search(lastName, firstName) {
    ...
    try {
      ...
      // Filter to only UTSA professors
      const schoolProfessors = edges.filter(edge => {
        const schoolName = edge.node.school?.name || '';
        const isMatch = schoolName.toLowerCase().includes('university of texas at san antonio');
        if (!PRODUCTION_MODE) console.log(`[RMP API] Checking: ${edge.node.firstName} ${edge.node.lastName} at ${schoolName} - Match: ${isMatch}`);
        return isMatch;
      });

      if (!PRODUCTION_MODE) console.log(`[RMP API] Found ${schoolProfessors.length} UTSA professors out of ${edges.length} total results`);

      if (uhProfessors.length === 0) {
        return { found: false, message: 'No UTSA professors found in results' };
      }

      // Try exact match first (in UTSA professors only)
      ...
      // Use best match if last name matches (in UTSA professors only)
