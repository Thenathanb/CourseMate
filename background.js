 ...
 */
const RMPProvider = {
  SCHOOL_ID: "U2Nob29sLTEyNzM=", // University of Texas at Dallas
  ...
  async search(lastName, firstName) {
    ...
    try {
      ...
      // Filter to only UTD professors
      const schoolProfessors = edges.filter(edge => {
        const schoolName = edge.node.school?.name || '';
        const isMatch = schoolName.toLowerCase().includes('university of texas at dallas');
        if (!PRODUCTION_MODE) console.log(`[RMP API] Checking: ${edge.node.firstName} ${edge.node.lastName} at ${schoolName} - Match: ${isMatch}`);
        return isMatch;
      });

      if (!PRODUCTION_MODE) console.log(`[RMP API] Found ${schoolProfessors.length} UTD professors out of ${edges.length} total results`);

      if (uhProfessors.length === 0) {
        return { found: false, message: 'No UTD professors found in results' };
      }

      // Try exact match first (in UTD professors only)
      ...
      // Use best match if last name matches (in UTD professors only)
 ...
 */
async function fetchProfessorData(professorName, school = 'University of Texas at Dallas') {
  try {
...
chrome.runtime.onInstalled.addListener(async () => {
  const defaults = {
    ...
    defaultSchool: 'University of Texas at Dallas',
